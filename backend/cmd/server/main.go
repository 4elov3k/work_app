...
func main() {
	...
	// API маршруты, сгруппированные по фичам через r.Group — оно не добавляет
	// сегмент пути (в отличие от r.Route), а просто обособляет middleware
	// каждой фичи от остальных на будущее.
	r.Route("/api", func(r chi.Router) {
		r.Get("/organization", h.GetOrganization)

		// Документы: контрагенты, договоры (+приложения), счета, акты,
		// услуги, парсинг сканов. Привязка контрагента к Redmine-проекту
		// живёт здесь же — это отношение самого контрагента.
		...
		// Redmine-синхронизация: проекты, дашборд менеджеров, контрольные
		// срезы/отчётные даты (control-events), загрузка PDF в проект.
		r.Group(func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Post("/zvonari/sync/stop", h.StopZvonariSync)
			...
			r.Get("/zvonari/calls/fraud-counts", h.GetFraudCounts)
			r.Get("/zvonari/callers/{id}/calls", h.GetCallerCalls)
			r.Get("/zvonari/calls/{id}/recording", h.GetCallRecording)
			r.Post("/zvonari/calls/{id}/transcribe", h.RetranscribeCall)
			r.Post("/zvonari/calls/{id}/analyze", h.AnalyzeCall)
