...
function filenameFromDisposition(disposition: string | null) {
  if (!disposition) return "invoice.xml"
  ...
  return asciiMatch?.[1] || "invoice.xml"
}

export default function DownloadXml({ invoiceId }: DownloadXmlProps) {
  const [loading, setLoading] = useState(false)

  const handleDownload = async () => {
    setLoading(true)
    try {
      const response = await fetch(`${API_BASE}/invoices/${invoiceId}/export/upd-xml`)
      if (!response.ok) {
  ...
  return (
    <Button type="button" variant="outline" onClick={handleDownload} disabled={loading}>
      ...
      {loading ? "Создание XML..." : "Скачать XML"}
    </Button>
  )
}
