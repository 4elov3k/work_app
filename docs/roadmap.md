...
- **Тренды по периодам** — fraud-категории и IQ-200 v1.2 регламент уже реализованы; следующий логичный шаг — сравнение периодов между собой на одном экране, не только снимок "за период". План:
  1. Backend: агрегирующий эндпоинт, принимающий 2 периода (текущий/прошлый), возвращающий те же метрики что и снимок за период (fraud rate, доля unanalyzed, объём звонков, распределение outcome) для обоих периодов
  2. Frontend: компонент сравнения на странице `/zvonari` — либо side-by-side карточки, либо дельта-индикаторы (%change) рядом с текущими метриками
  3. Решить перед стартом: сравнение произвольных периодов (гибче, сложнее в UI) или только "текущий vs предыдущий такой же" (проще, полезнее по умолчанию) — начать со второго
